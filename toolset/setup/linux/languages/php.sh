...
fw_get http://php.net/distributions/php-5.4.33.tar.bz2 -O php-5.4.33.tar.bz2
fw_untar php-5.4.33.tar.bz2
ls
mv php-5.4.33 php
ls
cd php

./configure --prefix=$IROOT/php-5.4.33 --with-pdo-mysql --with-mysql --with-mcrypt --enable-intl --enable-mbstring --enable-fpm --with-fpm-user=www-data --with-fpm-group=www-data --with-openssl
make
make install
cd ..

cp $FWROOT/config/php.ini $IROOT/php-5.4.33/lib/php.ini
cp $FWROOT/config/php-fpm.conf $IROOT/php-5.4.33/lib/php-fpm.conf
...
# Apc.so
$IROOT/php-5.4.33/bin/pecl config-set php_ini $IROOT/php-5.4.33/lib/php.ini
printf "\n" | $IROOT/php-5.4.33/bin/pecl install -f apc-beta

# yaf.so
printf "\n" | $IROOT/php-5.4.33/bin/pecl install -f yaf
...
export PATH=$IROOT/php-5.4.33/bin:$IROOT/php-5.4.33/sbin:$PATH
git clone git://github.com/phalcon/cphalcon.git
cd cphalcon
git checkout phalcon-v1.3.2
cd build/64bits 
$IROOT/php-5.4.33/bin/phpize
./configure --prefix=$IROOT/php-5.4.33 --enable-phalcon
make
make install
